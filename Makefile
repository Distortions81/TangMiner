...
# Reproduces the hardware-validated Mega 138K place/route configuration.
...
GOWIN_OPTION_SUFFIX := $(if $(call truthy,$(SPINAL_FULLY_UNROLLED)),_unrolled1,)$(if $(call truthy,$(SPINAL_HALF_UNROLLED)),_halfunrolled1,)$(if $(call truthy,$(SPINAL_ROUND_SKIP)),_skip1,)$(if $(call truthy,$(SPINAL_HOST_ROUND_SKIP)),_hostskip1,)$(if $(call truthy,$(SPINAL_CONTINUOUS_RESULTS)),_continuous1,)$(if $(call truthy,$(SPINAL_TWO_ROUNDS_PER_CYCLE)),_2round1,)$(if $(call truthy,$(SPINAL_TWO_ROUND_PIPELINE)),_2rpipe1,)$(if $(call truthy,$(SPINAL_TWO_PHASE_ROUND_PIPELINE)),_2phasepipe1,)$(if $(call truthy,$(SPINAL_REGISTER_PASS_OUTPUTS)),_regpass1,)$(if $(call truthy,$(SPINAL_REGISTER_COMPRESSOR_OUTPUTS)),_regcomp1,)$(if $(call truthy,$(SPINAL_REGISTER_FIRST_PASS_FEEDFORWARD)),_regff1,)$(if $(call truthy,$(SPINAL_REGISTER_ROUND_CONSTANT)),_regk1,)$(if $(call truthy,$(SPINAL_MINIMIZE_SHA_RESET)),_minreset1,)$(if $(call truthy,$(SPINAL_CSA_ROUND)),_csa1,)$(if $(call truthy,$(SPINAL_CSA_SCHEDULE)),_csasch1,)$(if $(call truthy,$(SPINAL_BALANCED_ROUND_ADDER)),_baladd1,)$(if $(call truthy,$(SPINAL_SHARE_JOB_STATE)),_sharejob1,)$(if $(filter-out auto,$(SPINAL_UNROLLED_DELAY_STYLE)),_delay$(SPINAL_UNROLLED_DELAY_STYLE),)$(if $(filter 0 false FALSE no NO off OFF,$(strip $(SPINAL_SHARED_K))),_localK,)
...
SPINAL_LANE_PERIOD_CYCLES ?= $(if $(call truthy,$(SPINAL_FULLY_UNROLLED)),1,$(if $(call truthy,$(SPINAL_HALF_UNROLLED)),2,$(shell expr $(SPINAL_BASE_PERIOD_CYCLES) \* $(SPINAL_ROUND_CYCLE_MULTIPLIER) + $(SPINAL_PASS_OUTPUT_CYCLES) + $(SPINAL_COMPRESSOR_OUTPUT_CYCLES) + $(SPINAL_FIRST_PASS_FEEDFORWARD_CYCLES))))
...
$(SPINAL_CONFIG): FORCE | $(BUILD)/.dir
	...
	{ \
	  ...
	  echo "half_unrolled=$(SPINAL_HALF_UNROLLED)"; \
	  ...
	  echo "continuous_results=$(SPINAL_CONTINUOUS_RESULTS)"; \
...
$(SPINAL_SRC): src/main/scala/tangminer/TangMiner.scala src/main/scala/tangminer/HalfUnrolledPipeline.scala build.sbt project/build.properties $(SPINAL_CONFIG) | $(BUILD)/.dir
	mkdir -p $(SPINAL_DIR)

...
$(error Unsupported SPINAL_CLOCK_PROFILE '$(SPINAL_CLOCK_PROFILE)'. Use 27m, 54m, 57m, 58m5, 60m75, 67m5, 81m, 84m, 85m5, 90m, 100m286, 111m, 120m, 123m, 124m875, 126m, 130m5, 135m, or 150m)
endif
...
GOWIN_OPTION_SUFFIX := $(if $(call truthy,$(SPINAL_ROUND_SKIP)),_skip1,)$(if $(call truthy,$(SPINAL_REGISTER_PASS_OUTPUTS)),_regpass1,)$(if $(call truthy,$(SPINAL_REGISTER_COMPRESSOR_OUTPUTS)),_regcomp1,)$(if $(call truthy,$(SPINAL_REGISTER_ROUND_CONSTANT)),_regk1,)$(if $(call truthy,$(SPINAL_MINIMIZE_SHA_RESET)),_minreset1,)$(if $(call truthy,$(SPINAL_CSA_ROUND)),_csa1,)$(if $(filter 0 false FALSE no NO off OFF,$(strip $(SPINAL_SHARED_K))),_localK,)
...
SPINAL_BASE_PERIOD_CYCLES := $(if $(call truthy,$(SPINAL_ROUND_SKIP)),61,64)
SPINAL_ROUND_CYCLE_MULTIPLIER := $(if $(call truthy,$(SPINAL_THREE_CYCLE_ROUND)),3,$(if $(call truthy,$(SPINAL_TWO_CYCLE_ROUND)),2,1))
SPINAL_PASS_OUTPUT_CYCLES := $(if $(call truthy,$(SPINAL_REGISTER_PASS_OUTPUTS)),1,0)
...
SPINAL_LANE_PERIOD_CYCLES ?= $(shell expr $(SPINAL_BASE_PERIOD_CYCLES) \* $(SPINAL_ROUND_CYCLE_MULTIPLIER) + $(SPINAL_PASS_OUTPUT_CYCLES) + $(SPINAL_COMPRESSOR_OUTPUT_CYCLES))
...
sim-cocotb: $(SPINAL_SIM_SRC) check-cocotb
	PATH="$(TOOLBIN):$$PATH" $(MAKE) -C sim/cocotb SIM=$(SIM) PYTHON_BIN="$(abspath $(PYTHON))" RTL_SOURCES="$(abspath $(SPINAL_SIM_SRC))" EXTRA_COMPILE_ARGS= CLKS_PER_BIT=8 LANE_COUNT=$(SPINAL_LANES) HARDWARE_CLOCK_HZ=$(SPINAL_CLOCK_HZ) EXPECTED_LANE_PERIOD_CYCLES=$(SPINAL_LANE_PERIOD_CYCLES)

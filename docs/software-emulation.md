...
- The measured RTL cadence matches one tested nonce every
  `lane_period_cycles / lane_count` fabric clocks. The default 20K build uses
  `lane_period_cycles=65` because `SPINAL_REGISTER_PASS_OUTPUTS=1` adds a
  pass-boundary fence to the 64-cycle SHA cadence. Unfenced full-64 builds use
  `64`; experimental `SPINAL_ROUND_SKIP=1` uses a 61-cycle base cadence.

...
/** Direct, UART-free wrapper for the two-cycle folded SHA256d pipeline. */
class BitcoinHashHalfUnrolledPipelineSimTop extends Component {
  noIoPrefix()
  ...
  def makePipeline(fixedCandidateMode: Int): BitcoinHashHalfUnrolledPipeline =
    new BitcoinHashHalfUnrolledPipeline(
...
object GenerateHalfUnrolledPipelineSimVerilog extends App {
  val targetDirectory = sys.env.getOrElse("TANGMINER_HALF_UNROLLED_SIM_DIR", "build/half-unrolled-pipeline-sim")
  SpinalConfig(
    targetDirectory = targetDirectory,
    defaultConfigForClockDomains = ClockDomainConfig(resetKind = BOOT)
  ).generateVerilog(new BitcoinHashHalfUnrolledPipelineSimTop)
}
